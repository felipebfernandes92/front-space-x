  ...
  return (
    <div className={styles.container}>
      ...
      <main className="container">
        ...
        <BoxLaunchs>
          <Launch>
            <p className="name">{props.nextLaunch.name}</p>
        ...
        <BoxLaunchs>
          <Launch>
            <p className="name">{props.latestLaunch.name}</p>
...
export async function getServerSideProps() {
  const resNext = await fetch('http://api-space-x.herokuapp.com/launchs/next')
  const nextLaunch = await resNext.json()

  const resLatest = await fetch('http://api-space-x.herokuapp.com/launchs/latest')
